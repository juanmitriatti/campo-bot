...
const PROFILE_FIELDS_WHITELIST = ['name', 'email', 'city', 'address', 'postal_code', 'province'] as const;

export class AuthRepository {
  async findByEmail(email: string): Promise<UserWithPassword | null> {
    const { rows } = await pool.query(
      `SELECT id, name, email, role, city, address, postal_code, province, plan_id, password_hash
  ...
  async getUserById(userId: number): Promise<AuthUser | null> {
    const { rows } = await pool.query(
      `SELECT id, name, email, role, city, address, postal_code, province, plan_id
  ...
  async createUser({ name, email, passwordHash, planId }: {
    ...
    const { rows } = await pool.query(
      `INSERT INTO users (name, email, password_hash, role, plan_id)
       ...
       RETURNING id, name, email, role, city, address, postal_code, province, plan_id`,
