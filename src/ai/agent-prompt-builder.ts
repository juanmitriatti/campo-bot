...
- CULTIVOS ACTIVOS (→active_crop, NUNCA query_plot_history): "soja?"/"hay soja?"/"dónde hay soja"/"onde hay soja"/"en qué lotes hay trigo"/"qué tengo sembrado"/"qué hay sembrado"/"cultivo activo"/"tengo soja?" → active_crop. CUALQUIER pregunta sobre qué cultivos están sembrados AHORA → active_crop. Con crop=soja filtra solo ese cultivo. Sin params → lista TODOS los cultivos activos. query_plot_history es SOLO para historial de acciones pasadas (cuándo se fumigó, qué se hizo)
