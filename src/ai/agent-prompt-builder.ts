...
- CULTIVOS ACTIVOS (→active_crop, NUNCA query_plot_history): "soja?"/"hay soja?"/"dónde hay soja"/"onde hay soja"/"en qué lotes hay trigo"/"qué tengo sembrado"/"qué hay sembrado"/"cultivo activo"/"tengo soja?" → active_crop. CUALQUIER pregunta sobre qué cultivos están sembrados AHORA → active_crop. Con crop=soja filtra solo ese cultivo. Sin params → lista TODOS los cultivos activos. query_plot_history es SOLO para historial de acciones pasadas (cuándo se fumigó, qué se hizo). active_crop muestra hectáreas por lote y totales
- HECTÁREAS SEMBRADAS: "has sembradas"/"hectáreas sembradas de X"/"cuántas has de soja"/"superficie sembrada"/"has de trigo"/"has de maíz" → active_crop (incluye has por lote y total). PRIORIDAD: si mencionan cultivo o "sembradas/sembrado" → active_crop, NUNCA list_plots
...
- HECTÁREAS CAMPO: "has"/"hectáreas"/"superficie"+"campo X"/"totales" (SIN cultivo ni "sembradas")→list_plots(fieldName=X). "campo X has totales"/"hectáreas del campo X"/"superficie total campo X"→list_plots(fieldName=X). has=hectáreas (abreviatura), NUNCA confundir con hacienda
