...
export const TOOL_DEFINITIONS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        buyer: { type: 'string', description: 'Comprador / acopio de una venta de grano ("le vendí a Cargill", "ACA"). Descuenta del saldo entregado.' },
        price_status: { type: 'string', enum: ['fijado', 'a_fijar'], description: '"a fijar"/"sin fijar precio" → a_fijar. Precio cerrado → fijado. Omitir si no lo dijo.' },
        field: FIELD_PROP,
        plot: PLOT_PROP,
        event_date: DATE_PROP,
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        hectares: { type: 'number', description: 'Hectáreas cosechadas EN ESTE mensaje, si las dijo ("cosechamos 40 ha del Norte"). Avance parcial; omitir si no las nombró.' },
        loads: {
          type: 'array',
          ...
          items: {
            ...
            properties: {
              ...
              gross_weight_kg: { type: 'number', description: 'Bruto de balanza ("bruto 45.200"). Con tara, el peso es bruto − tara.' },
              tare_kg: { type: 'number', description: 'Tara del camión ("tara 14.000").' },
              acopio_weight_kg: { type: 'number', description: 'Kg que pesó el destino/acopio ("en Cargill pesó 30.900").' },
              carta_porte: { type: 'string', description: 'Nº de carta de porte, si lo dijo.' },
              ctg: { type: 'string', description: 'CTG (código de trazabilidad), si lo dijo.' },
            },
...

  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        without_ctg: { type: 'boolean', description: 'true = solo camiones SIN CTG / carta de porte ("qué camiones no tienen CTG").' },
        // Presentation
        view: { type: 'string', enum: ['detail', 'aggregate', 'max', 'min', 'avg', 'top_locations', 'compare', 'rank', 'volume', 'last', 'balance'], description: 'detail=lista (default). aggregate=resumen totales+conteos. max=la carga con mayor X. min=la más baja. avg=promedio de X. top_locations=ranking por lote/campo/chofer/destinatario/cultivo. compare=2 grupos side-by-side. rank=top N. volume=toneladas por cultivo/chofer/destinatario (es sinónimo de top_locations con metric=weight). balance=SALDO en el acopio: entregado (neto) − vendido − retirado ("cuánta soja tengo en Cargill", "qué me queda en el acopio").' },
  ...
  {
    name: 'log_harvest_costs',
    description: 'Costo de COSECHAR: contratista (% del rinde o $/ha o total) y/o flete ($/tn o total). "la cosecha del Norte salió 8%", "el contratista cobró 45.000 por ha", "flete 18.000 la tonelada". NO es log_expense genérico: crea los gastos Cosecha/Flete sobre la campaña del lote.',
    input_schema: {
      type: 'object',
      properties: {
        plot: PLOT_PROP,
        field: FIELD_PROP,
        crop: { type: 'string', description: 'Cultivo, si lo nombró.' },
        contractor_pct: { type: 'number', description: '% del rinde que cobra el contratista ("8%").' },
        contractor_per_ha: { type: 'number', description: '$ por hectárea del contratista.' },
        contractor_total: { type: 'number', description: '$ total del contratista.' },
        freight_per_tn: { type: 'number', description: '$ por tonelada de flete.' },
        freight_total: { type: 'number', description: '$ total de flete.' },
        currency: { type: 'string', enum: ['ARS', 'USD'], description: 'Moneda. Default ARS.' },
      },
      required: [],
    },
  ...
  {
    name: 'edit_harvest_load',
    description: 'Corregir UN camión ya cargado, identificado por chofer: "el camión de Pérez eran 30.320", "la carga de Gómez fue a ACA", "el CTG de Pérez es 123". NO es harvest_crop (eso agrega camiones nuevos).',
    input_schema: {
      type: 'object',
      properties: {
        driver_name: { type: 'string', description: 'Chofer del camión a corregir.' },
        plot: PLOT_PROP,
        weight_kg: { type: 'number', description: 'Kg corregidos (31.320 = 31320).' },
        destinatario: { type: 'string', description: 'Destino corregido.' },
        humidity_pct: { type: 'number', description: 'Humedad corregida.' },
        truck_plate: { type: 'string', description: 'Patente.' },
        acopio_weight_kg: { type: 'number', description: 'Kg que pesó el acopio.' },
        carta_porte: { type: 'string', description: 'Nº carta de porte.' },
        ctg: { type: 'string', description: 'CTG.' },
      },
      required: ['driver_name'],
    },
  },
  {
    name: 'set_expected_yield',
    description: 'Rinde ESPERADO precosecha para un lote: "espero 40 qq/ha en el Norte", "estimo 3.500 kg/ha de soja". NO es harvest_crop (no cosechó todavía) ni campaign_stats (consulta).',
    input_schema: {
      type: 'object',
      properties: {
        plot: PLOT_PROP,
        field: FIELD_PROP,
        crop: { type: 'string', description: 'Cultivo, si lo nombró.' },
        kg_per_ha: { type: 'number', description: 'Rinde esperado en kg/ha. "40 qq/ha" → 4000. "3,5 tn/ha" → 3500.' },
      },
      required: ['kg_per_ha'],
    },
  },
  {
    name: 'log_grain_withdrawal',
    description: 'RETIRO de grano entregado (no venta): "retiré 30 tn de soja de Cargill", "saqué 10 tn del silo para consumo". Baja el saldo del acopio/silo. Vender es log_income.',
    input_schema: {
      type: 'object',
      properties: {
        crop: { type: 'string', description: 'Grano retirado.' },
        quantity: QUANTITY_PROP,
        unit: UNIT_PROP,
        destinatario: { type: 'string', description: 'De dónde se retira (acopio, silo).' },
        reason: { type: 'string', description: 'Motivo, si lo dijo (consumo, traslado, semilla).' },
        event_date: DATE_PROP,
      },
...

  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        view: { type: 'string', enum: ['full', 'yield', 'rank'], description: "'yield' = respuesta CORTA solo con el rinde (kg, tn, kg/ha). Usar cuando preguntan específicamente cuánto rindió / rendimiento / cuántos kg-tn sacó / conversión de unidades del rinde. 'rank' = COMPARAR entre campañas: 'qué lote rindió/dejó más', 'cuál fue el más caro', 'me fue mejor con soja o con maíz', 'costo por tonelada de cada lote'. 'full' (default) = ficha completa de UNA campaña — para 'cómo viene la campaña', 'resumen', 'rentabilidad'." },
        group_by: { type: 'string', enum: ['plot', 'crop'], description: "Solo con view='rank'. 'plot' (default) = una fila por campaña de cada lote. 'crop' = suma los lotes del mismo cultivo ('me fue mejor con soja o con maíz')." },
        metric: { type: 'string', enum: ['margin', 'yield_kg_ha', 'cost_per_ha', 'cost_per_tn'], description: "Solo con view='rank'. Qué se rankea: margin (default, ingresos - gastos) · yield_kg_ha (rinde) · cost_per_ha · cost_per_tn. En las de costo gana el MENOR." },
        top_n: { type: 'number', description: "Solo con view='rank'. Cuántos mostrar ('los 3 mejores lotes')." },
      },
      required: [],
    },
  },
  ...
  // ========================
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        period: { type: 'string', enum: ['today', 'week', 'last_week', 'month', 'last_month', 'year', 'last_year', 'all'], description: 'Período calendario. today=hoy · week=esta semana (arranca lunes) · last_week=la semana pasada completa · month=este mes · last_month=el mes pasado completo · year=este año · last_year=el año pasado · all=todo el historial.' },
        search: { type: 'string', description: 'Buscar texto dentro de las notas (ej "pulgón").' },
        desde: { type: 'string', description: 'YYYY-MM-DD inicio (alternativa a period).' },
        hasta: { type: 'string', description: 'YYYY-MM-DD fin.' },
        category: { type: 'string', enum: ['sanidad', 'malezas', 'nutricion', 'fenologia', 'clima', 'general'], description: 'Filtrar por categoría de la nota.' },
        view: { type: 'string', enum: ['detail', 'aggregate', 'top_locations', 'last'], description: "detail (default) = lista las notas. aggregate = CUÁNTAS por categoría/lote/mes ('cuántas notas tengo', 'de qué anoté más'). top_locations = qué lote/campo tiene más notas. last = solo la última." },
        group_by: { type: 'string', enum: ['category', 'plot', 'field', 'month'], description: "Con view='aggregate'. Default category." },
        aggregate_metric: { type: 'string', enum: ['count'], description: "Único aplicable a notas libres: count." },
        sort_by: { type: 'string', enum: ['date', 'count'], description: 'Orden. date (default en detail), count (default en aggregate).' },
        sort_desc: { type: 'boolean', description: 'Descendente (default true).' },
        top_n: { type: 'number', description: 'Cuántos resultados ("las 3 últimas", "los 5 lotes con más notas").' },
        inherit: { type: 'boolean', description: 'true cuando el usuario REFINA la consulta anterior de observaciones ("¿y en La Esperanza?", "solo las de sanidad"). Hereda los filtros previos.' },
      },
      required: [],
    },
  },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        // Presentation
        view: { type: 'string', enum: ['detail', 'aggregate', 'max', 'min', 'avg', 'top_locations', 'compare', 'rank', 'last'], description: 'detail=lista (default). aggregate=resumen agregado. max=el más alto en una métrica. min=el más bajo. avg=promedio. top_locations=ranking por lote/campo. compare=2 lotes/períodos side by side. rank=top N por una métrica con orden.' },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        movement_type: { type: 'string', enum: ['entrada', 'salida', 'muerte', 'nacimiento', 'transferencia', 'recategorizacion', 'ajuste'], description: "Filtrar por tipo. 'nacimiento' para «cuándo nacieron», 'transferencia' para «cuándo los moví/pasé de lote», 'salida' para ventas, 'muerte' para bajas." },
        period: { type: 'string', enum: ['today', 'week', 'last_week', 'month', 'last_month', 'year', 'last_year', 'all'], description: 'Período calendario. Alternativa a desde/hasta.' },
        view: { type: 'string', enum: ['detail', 'last', 'aggregate'], description: "detail (default) = totales + listado. last = SOLO el/los más reciente(s) con la fecha — usar cuando preguntan CUÁNDO ocurrió algo. aggregate = solo los totales por tipo." },
        top_n: { type: 'number', description: "Cuántos traer. Con view='last', cuántos movimientos recientes (default 1)." },
        inherit: { type: 'boolean', description: 'true cuando el usuario REFINA la consulta anterior de movimientos («¿y en el otro lote?», «¿y los nacimientos?»).' },
      },
      required: [],
    },
  },
