...

  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        // Presentation
        view: { type: 'string', enum: ['detail', 'aggregate', 'max', 'min', 'avg', 'top_locations', 'compare', 'rank', 'volume', 'last'], description: 'detail=lista (default). aggregate=resumen totales+conteos. max=la carga con mayor X. min=la más baja. avg=promedio de X. top_locations=ranking por lote/campo/chofer/destinatario/cultivo. compare=2 grupos side-by-side. rank=top N. volume=toneladas por cultivo/chofer/destinatario (es sinónimo de top_locations con metric=weight).' },
