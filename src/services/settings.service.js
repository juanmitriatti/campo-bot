...
const SETTING_DEFINITIONS = {
  ...
  AUDIO_ECHO_TRANSCRIPT: { default: 'true', type: 'boolean', group: 'audio', label: 'Eco de transcripción de audio', description: 'Tras transcribir un audio, el bot muestra "🎙️ Entendí: «...»" antes de procesar. El usuario ve qué entendió el STT y puede corregir — la mayoría de los errores de audio son transcripciones malas invisibles. Aplica a WhatsApp y Telegram.' },
  ...
  AGENT_MAX_TOKENS: { default: '1500', type: 'number', group: 'ai', label: 'Max tokens AI Agent', description: 'Tokens máximos para respuesta del agent. 1500 da margen para compounds de 4-5 tools; 400 truncaba mensajes largos (stop_reason=max_tokens).' },
  AGENT_TIMEOUT_MS: { default: '12000', type: 'number', group: 'ai', label: 'Timeout AI Agent (ms)', description: 'Timeout TOTAL del agent en ms (incluye los reintentos del SDK ante 429/529/5xx). 12000 da margen para 1-2 reintentos con backoff; con 8000 los reintentos se cortaban.' },
...
- Usá "lucas"=miles, "palos"=millones si el usuario los usa.`, type: 'string', group: 'ai', label: 'System prompt del fallback conversacional', description: 'Instrucciones que recibe el modelo para generar respuestas conversacionales. Define personalidad y límites del bot. NO incluir lista enumerada de capacidades — usar el comando "ayuda" para eso.' },
  ...
  TIPS_ENABLED: { default: 'true', type: 'boolean', group: 'bot', label: 'Tips contextuales de primera vez', description: 'Kill switch de los tips de descubrimiento ("💡 También podés mandarme un audio..."). Tras la primera acción de cada tipo, el bot enseña UNA capacidad relacionada. Máximo TIPS_MAX_PER_DAY por día por usuario; cada tip se muestra una sola vez. El usuario puede desactivarlos con "no más tips". Cambios aplican en ≤5 min (cache de settings).' },
  TIPS_MAX_PER_DAY: { default: '1', type: 'number', group: 'bot', label: 'Máx tips por día por usuario', description: 'Tope diario de tips contextuales por usuario. Default 1 (recomendado: más se siente spam). 0 NO desactiva — usá TIPS_ENABLED.' },
  PROACTIVE_ALERTS_ENABLED: { default: 'false', type: 'boolean', group: 'bot', label: 'Alertas proactivas', description: 'Kill switch de las alertas proactivas: clima diario (lluvia/viento/ventana seca), recordatorio de monitoreo, escalamiento de plagas, hectáreas faltantes, stock bajo y fenología. Con esto en false los cron corren pero no envían nada. Además cada usuario puede optar por no recibirlas con "no más alertas" (user_settings.alerts_enabled).' },
  MONTHLY_INSIGHTS_ENABLED: { default: 'true', type: 'boolean', group: 'bot', label: 'Tendencias en resumen mensual', description: 'Agrega al resumen mensual un bloque "📈 Tendencias" con las categorías de gasto que más variaron vs el mes anterior (ej: "Gasoil: subió 18%").' },
  MONTHLY_INSIGHTS_MIN_PCT: { default: '15', type: 'number', group: 'bot', label: 'Tendencias: variación mínima (%)', description: 'Umbral de variación porcentual por categoría para aparecer en el bloque de tendencias del resumen mensual. Default 15 (%).' },
  WHISPER_PRICE_PER_MINUTE: { default: '0.006', type: 'number', group: 'audio', label: 'Precio Whisper (USD/min)', description: 'Precio por minuto de transcripción usado para calcular cost_usd en audio_transcription_logs. Default 0.006 (whisper-1 de OpenAI). Actualizar si cambia el pricing o el proveedor.' },
  AGRONOMY_QA_ENABLED: { default: 'true', type: 'boolean', group: 'ai', label: 'Consultas de conocimiento agronómico', description: 'Kill switch de la tool agronomy_question: respuestas educativas de agronomía/ganadería ("¿qué significa V6?", "¿cuándo conviene sembrar trigo?") vía llamada dedicada a Claude sin tools (read-only estructural). Con false, el bot responde que la consulta educativa no está disponible.' },
  AGRONOMY_QA_MODEL: { default: 'claude-haiku-4-5-20251001', type: 'string', group: 'ai', label: 'Modelo para consultas agronómicas', description: 'Modelo de la llamada dedicada de agronomy_question. Independiente de AGENT_MODEL.' },
  AGRONOMY_QA_MAX_TOKENS: { default: '600', type: 'number', group: 'ai', label: 'Max tokens consulta agronómica', description: 'Tope de tokens de salida para las respuestas educativas. Default 600 (respuestas de chat, no ensayos).' },
  AGRONOMY_QA_TIMEOUT_MS: { default: '10000', type: 'number', group: 'ai', label: 'Timeout consulta agronómica (ms)', description: 'Timeout de la llamada dedicada de agronomy_question. Default 10000.' },
  DATA_ANALYSIS_ENABLED: { default: 'true', type: 'boolean', group: 'ai', label: 'Análisis de datos con IA (dashboard)', description: 'Kill switch del tab "Análisis de datos" del dashboard: el usuario pregunta en lenguaje natural sobre sus propios números (gastos, ingresos, actividades, cosechas, lluvias, hacienda, stock) de una campaña y Claude responde en markdown. Llamada dedicada sin tools, read-only. Con false la ruta devuelve 503 con mensaje honesto.' },
  DATA_ANALYSIS_MODEL: { default: 'claude-sonnet-5', type: 'string', group: 'ai', label: 'Modelo del análisis de datos', description: 'Modelo de la llamada dedicada del análisis. Independiente de AGENT_MODEL. Sonnet 5 por defecto (equilibrio calidad/costo); claude-opus-5 para máximo análisis, claude-haiku-4-5-20251001 para mínimo costo. El costo por consulta se calcula con la tabla de src/ai/model-pricing.ts.' },
  DATA_ANALYSIS_EFFORT: { default: 'medium', type: 'select', options: ['low', 'medium', 'high'], group: 'ai', label: 'Esfuerzo de razonamiento del análisis', description: 'output_config.effort de la llamada (low/medium/high). Más esfuerzo = mejor análisis comparativo, más tokens de salida y más latencia.' },
  DATA_ANALYSIS_MAX_TOKENS: { default: '3000', type: 'number', group: 'ai', label: 'Max tokens del análisis', description: 'Tope de tokens de salida por respuesta. Si se corta, la respuesta lleva la nota "(respuesta recortada por largo)".' },
  DATA_ANALYSIS_TIMEOUT_MS: { default: '60000', type: 'number', group: 'ai', label: 'Timeout del análisis (ms)', description: 'Timeout de la llamada. La ruta no streamea: si el proxy de prod corta antes, bajar este valor.' },
  DATA_ANALYSIS_MAX_DATA_CHARS: { default: '60000', type: 'number', group: 'ai', label: 'Presupuesto del bloque de datos (chars)', description: 'Tamaño máximo del JSON de datos del usuario que se manda al modelo (~15k tokens con 60000). Si se excede, las listas se recortan en orden fijo y se avisa al usuario.' },
  DATA_ANALYSIS_MAX_ROWS_PER_LIST: { default: '150', type: 'number', group: 'ai', label: 'Filas máximas por lista de datos', description: 'Tope de filas crudas por lista (gastos, ingresos, eventos, cargas, lluvias, stock, hacienda) antes del presupuesto de chars. Las más recientes primero.' },
  DATA_ANALYSIS_MAX_HISTORY_TURNS: { default: '6', type: 'number', group: 'ai', label: 'Turnos de repregunta aceptados', description: 'Pares pregunta/respuesta previos que se mandan al modelo para el seguimiento. El hilo vive en la pantalla, no se guarda.' },
  DATA_ANALYSIS_MAX_QUESTION_CHARS: { default: '1000', type: 'number', group: 'ai', label: 'Largo máximo de la pregunta', description: 'Caracteres máximos de una pregunta del análisis de datos.' },
  TRIAL_DRIP_ENABLED: { default: 'true', type: 'boolean', group: 'bot', label: 'Drip de descubrimiento del trial', description: 'Kill switch de los mensajes proactivos durante el trial (uno por día configurado en TRIAL_DRIP_DAYS, mostrando una capacidad del bot con ejemplo). Respeta el opt-out de tips ("no más tips" apaga ambos). Solo usuarios con suscripción en trial.' },
  TRIAL_DRIP_DAYS: { default: '2,5,8,11', type: 'string', group: 'bot', label: 'Drip: días del trial', description: 'Días del trial en que se envía cada mensaje (0 = día del registro), separados por coma. La posición N corresponde al mensaje N de TRIAL_DRIP_MESSAGES.' },
  TRIAL_DRIP_HOUR: { default: '10', type: 'number', group: 'bot', label: 'Drip: hora de envío (AR)', description: 'Hora argentina (0-23) en que se envían los mensajes del drip. El tick corre cada hora y solo dispara en esta.' },
  TRIAL_DRIP_MESSAGES: { default: '💡 ¿Sabías que me podés mandar *audios*? Contame arriba de la camioneta: _"gasté 80 mil en gasoil"_ — lo entiendo igual que el texto.\n---\n📸 Sacale una *foto a una factura o remito* y te cargo los gastos o el stock solos. Probá con la próxima que te llegue.\n---\n🐄 ¿Tenés hacienda? Decime _"compré 40 terneros a 500 mil por cabeza"_ o _"vacuné las vacas contra aftosa"_. Y preguntame lo que sea: _"¿cuánto gasté este mes?"_, _"¿a cuánto está la soja?"_\n---\n📊 Pedime _"reporte agro"_ y te armo un PDF con todo lo de tus lotes. Y si el bot te está sirviendo: escribí *plan* para ver las opciones — tu prueba está por terminar.', type: 'string', group: 'bot', label: 'Drip: mensajes', description: 'Mensajes del drip, separados por una línea "---". El mensaje N se envía el día N de TRIAL_DRIP_DAYS. Formato WhatsApp/Telegram (*negrita*, _cursiva_).' },
  ...
  WHATSAPP_FLOW_ID_SOW: { default: '', type: 'string', group: 'bot', label: 'WhatsApp Flow ID — formulario de siembra', description: 'ID del Flow publicado en Meta (WhatsApp Manager → Flows) para el formulario de siembra. Vacío = no se ofrece formulario por WhatsApp (el chat sigue funcionando igual). Lo genera y guarda `npx tsx src/scripts/publish-whatsapp-flows.ts --publish --save-settings`. Cambios aplican en ≤5 min (cache de settings).' },
  WHATSAPP_FLOW_ID_HARVEST: { default: '', type: 'string', group: 'bot', label: 'WhatsApp Flow ID — formulario de cosecha', description: 'ID del Flow publicado en Meta para el formulario de cosecha. Vacío = no se ofrece formulario por WhatsApp. Mismo script que el de siembra.' },
  WHATSAPP_FLOW_ID_EXPENSE: { default: '', type: 'string', group: 'bot', label: 'WhatsApp Flow ID — formulario de gasto', description: 'ID del Flow publicado en Meta para el formulario de gasto (monto, moneda, categoría, lote o campo, fecha, detalle). Vacío = no se ofrece por WhatsApp. Lo genera el mismo script que siembra/cosecha.' },
  WHATSAPP_FLOW_ID_INCOME: { default: '', type: 'string', group: 'bot', label: 'WhatsApp Flow ID — formulario de ingreso', description: 'ID del Flow publicado en Meta para el formulario de ingreso. Vacío = no se ofrece por WhatsApp.' },
  WHATSAPP_FLOW_ID_ACTIVITY: { default: '', type: 'string', group: 'bot', label: 'WhatsApp Flow ID — formulario de labor', description: 'ID del Flow publicado en Meta para el formulario de labor (fumigación, fertilización, labranza, riego). Vacío = no se ofrece por WhatsApp.' },
  WHATSAPP_FLOW_ID_LIVESTOCK: { default: '', type: 'string', group: 'bot', label: 'WhatsApp Flow ID — formulario de hacienda', description: 'ID del Flow publicado en Meta para el alta de hacienda (categoría, cabezas, raza, lote o corral, precio). Vacío = no se ofrece por WhatsApp.' },
  SUGGESTIONS_ENABLED: { default: 'true', type: 'boolean', group: 'bot', label: 'Sugerencias "¿Y ahora?"', description: 'Kill switch de los botones de próximos pasos que se agregan después de cada acción (gasto guardado → Resultado mes / Cargar factura…). Con esto en false no se envía ninguna sugerencia; el resto de la respuesta no cambia. Aplica en ≤5 min (cache de settings).' },
  SUGGESTIONS_MAX_PER_DAY: { default: '0', type: 'number', group: 'bot', label: 'Sugerencias: tope por usuario y día', description: 'Máximo de mensajes "¿Y ahora?" por usuario por día (hora Argentina). 0 = sin tope (comportamiento histórico). Se cuenta sobre suggestion_events. Recomendado para empezar: 3.' },
  SUGGESTIONS_DISABLED_KEYS: { default: '', type: 'string', group: 'bot', label: 'Sugerencias: claves apagadas', description: 'Claves del catálogo que NO se envían, separadas por coma (ej: "report_shown, field_info_shown"). Claves válidas: expense_saved, income_saved, activity_logged, harvest_logged, observation_logged, rainfall_logged, livestock_logged, stock_logged, report_shown, weather_shown, field_created, plot_created, field_info_shown, query_result, activities_shown, rainfall_shown, livestock_shown, stock_shown, crop_empty, livestock_empty, stock_empty, default_menu.' },
  SUGGESTIONS_OVERRIDES: { default: '', type: 'string', group: 'bot', label: 'Sugerencias: overrides (JSON)', description: 'Reemplaza la terna de una o más claves sin deploy. JSON con la forma {"expense_saved":{"body":"¿Y ahora?","buttons":[{"id":"cmd_resumen_mensual","title":"📈 Resultado mes"}]}}. Cada id tiene que existir en el router de botones (los mismos ids del catálogo), 1 a 3 botones, títulos de hasta 20 caracteres (límite de WhatsApp). Una entrada inválida se ignora y se loguea [SUGGEST]; las válidas aplican igual.' },
  WHATSAPP_FLOW_MODE: { default: 'published', type: 'select', options: ['published', 'draft'], group: 'bot', label: 'WhatsApp Flows: modo', description: '`published` para producción. `draft` permite probar un Flow todavía no publicado, pero SOLO llega a los números de prueba de la app de Meta — con usuarios reales el mensaje falla. Volver a `published` después de probar.' },
  ...
  HARVEST_HUMIDITY_BASES: { default: '', type: 'string', group: 'agronomy', label: 'Humedad base por cultivo (JSON)', description: 'Humedad de recibo por cultivo para calcular la merma de cada camión. JSON como {"soja":13.5,"trigo":14,"maiz":14.5}. Vacío = defaults de la Cámara Arbitral (grain-merma.ts). Un acopio que liquida distinto se ajusta acá sin redeploy.' },
  HARVEST_MERMA_MANIPULEO_PCT: { default: '0.3', type: 'number', group: 'agronomy', label: 'Merma por manipuleo (%)', description: 'Puntos que suma el secado a la merma física por humedad. Default 0,3 (aproxima las tablas oficiales). 0 = solo pérdida de agua.' },
  HARVEST_COST_OFFER_ENABLED: { default: 'true', type: 'boolean', group: 'agronomy', label: 'Ofrecer cargar costo de cosecha', description: 'Tras registrar una cosecha con producción, el bot ofrece con un botón cargar el costo del contratista y el flete como gastos del lote. Apagar si molesta.' },
  RETRO_YIELD_WINDOW_DAYS: { default: '60', type: 'number', group: 'agronomy', label: 'Ventana de rinde retroactivo (días)', description: 'Cuántos días hacia atrás se busca una campaña cosechada SIN rinde (incluso ya cerrada) cuando el usuario carga el dato tarde: "cosechamos 110 tn de maíz en el Sur" sin campaña activa en ese lote aplica el rinde a esa campaña vieja en vez de rechazar el mensaje. Existe porque la liquidación del acopio suele llegar días o semanas después del cierre. Pasada la ventana, el dato ya no se puede cargar por chat. Subirla mucho aumenta el riesgo de matchear una campaña equivocada en lotes que rotan rápido; 60 (default) cubre la liquidación típica. Solo COMPLETA campañas sin rinde — nunca pisa un rinde ya cargado.' },
  ...
  GREETING_NO_PLOTS_MESSAGE: { default: 'Hola{{nombre}} 👋 Ya tenés tu campo *{{fieldName}}*.\n\nAhora creá un lote para poder registrar gastos e ingresos. Escribí algo como:\n🌾 *agregar lote Norte en campo {{fieldName}}*', type: 'string', group: 'mensajes', label: 'Saludo: usuario sin lotes', description: 'Mensaje cuando el usuario tiene campo pero no tiene lotes. Variables: {{nombre}}, {{botName}}, {{fieldName}}' },
  ...
  TG_LINKED_WELCOME_MESSAGE: { default: '✅ *Cuenta vinculada*\n\nYa podés usar *{{botName}}* por Telegram. Probá con *menu* o escribí algo como "gasté 50 mil en gasoil".', type: 'string', group: 'mensajes', label: 'Telegram vinculado: usuario con campos', description: 'Mensaje tras vincular Telegram cuando el usuario YA tiene campos creados. Variables: {{botName}}' },
  TG_LINKED_WELCOME_NO_FIELDS_MESSAGE: { default: '✅ *Cuenta vinculada*\n\nPara arrancar, contame de tu campo — podés decirme todo junto:\n\n👉 _"Tengo el campo La Esperanza en Pergamino con los lotes Norte y Sur"_\n\nY de ahí en más registrá lo que quieras como te salga: gastos, siembras, lluvias, hacienda… por texto o audio.', type: 'string', group: 'mensajes', label: 'Telegram vinculado: usuario sin campos', description: 'Mensaje tras vincular Telegram cuando el usuario todavía NO tiene ningún campo (recién registrado). Guía el alta del campo en un solo mensaje — NO sugerir acciones que rebotan sin campo (ej: registrar un gasto). Variables: {{botName}}' },
  // Single canonical confirmation per domain (the admin can override with multiple
  // variants line-by-line if they want variety). Default is one consistent phrase
  // so users learn to recognize "Gasto registrado" / "Ingreso registrado".
  EXPENSE_CONFIRMATIONS_MESSAGE: { default: '✅ Gasto registrado', type: 'string', group: 'mensajes', label: 'Pool confirmaciones de gasto', description: 'Mensajes de confirmación al registrar un gasto. Uno por línea, se elige uno al azar.' },
  INCOME_CONFIRMATIONS_MESSAGE: { default: '💰 Ingreso registrado', type: 'string', group: 'mensajes', label: 'Pool confirmaciones de ingreso', description: 'Mensajes de confirmación al registrar un ingreso. Uno por línea, se elige uno al azar.' },

  // Onboarding
  ONBOARDING_FIRST_PLOT_MESSAGE: { default: '🌾 *¡Campo y lotes creados, {{nombre}}!*\n\nYa podés registrar. Probá con tus propios lotes:\n\n💰 *"Gasté 50000 en gasoil"*\n🌱 *"Sembré soja en {{lote}}"* · *"Fumigué {{lote}} con glifosato"*\n🐄 *"Compré 40 vacas a 900 mil c/u"*\n🌧 *"Llovieron 25 mm"* · *"¿Va a llover?"*\n\n🎙 También podés mandarme un *audio* o la *foto de una factura* — los entiendo igual.\n\n👉 *Empezá ahora: contame el último gasto que hiciste.*\n\n_(Escribí *menú* para ver todo lo demás — reportes, stock, recordatorios, pizarra.)_', type: 'string', group: 'onboarding', label: 'Mensaje de bienvenida tras primer lote', description: 'Mensaje que se envía una sola vez cuando el usuario crea su primer lote. Variables: {{nombre}} (usuario), {{campo}} y {{lote}} (el campo y el primer lote recién creados — los ejemplos quedan copy-pasteables con los nombres reales).' },
  ...
  SUPPORT_CONTACT: { default: '', type: 'string', group: 'system', label: 'Contacto de soporte', description: 'Canal de soporte que el bot muestra al usuario (ej: "soporte@campo.bot" o "+54 9 11 xxxx-xxxx (WhatsApp)"). Aparece en el comando *plan*, en el mensaje de trial vencido y en ayuda. Vacío = no se muestra ninguna línea de soporte. BLOQUEANTE para cobrar: un usuario pago necesita dónde quejarse.' },
  ...
  SENTRY_DSN: { default: '', type: 'string', group: 'system', label: 'Sentry DSN (backend) — solo ENV', description: '⚠️ SOLO se lee de la variable de entorno al ARRANQUE del servidor (Sentry se inicializa antes de que la DB esté disponible) — cambiarlo acá NO tiene efecto. Configurarlo en Railway → Variables y redeployar. Si está vacío, Sentry queda deshabilitado y los errores van a error_logs y stdout.' },
