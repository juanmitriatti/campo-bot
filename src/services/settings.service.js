...
const SETTING_DEFINITIONS = {
  ...
  AGENT_OUTPUT_VALIDATION_ENABLED: { default: 'false', type: 'boolean', group: 'ai', label: 'Validación server-side del output del agente', description: 'Cuando true, descarta entidades inferidas por el agente (cultivo, lote, campo) que no estén soportadas por el texto del usuario. Apagado por default mientras se rueda gradualmente. Cada regla individual puede activarse vía sus propios settings (AGENT_VALIDATE_CROP, etc.).' },
  AGENT_VALIDATE_CROP: { default: 'false', type: 'boolean', group: 'ai', label: 'Validar crop del agente contra texto del usuario', description: 'Cuando true (y AGENT_OUTPUT_VALIDATION_ENABLED=true), descarta el param crop si el usuario no nombró ese cultivo (ni un sinónimo conocido) en su mensaje. Esto fuerza al handler a pedir el cultivo en vez de aceptar lo que el agente alucinó.' },
  AGENT_VALIDATE_PLOT_FIELD: { default: 'false', type: 'boolean', group: 'ai', label: 'Validar plot/field del agente contra texto del usuario', description: 'Cuando true (y AGENT_OUTPUT_VALIDATION_ENABLED=true), descarta el param plot/field en tools de actividad cuando el usuario no nombró ese lote/campo en su mensaje y no usó un pronombre explícito ("ahí", "ese lote", etc.). NO se aplica a tools de creación (add_field, add_plot, rename_*, delete_*).' },
