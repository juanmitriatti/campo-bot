...
## Paridad del dashboard, dedup por contenido y la confirmación que confirma (Sep 9, 2026)

Los cuatro puntos que quedaron abiertos tras el QA de siembra/cosecha (ver sección anterior), atacados de raíz:

- **Paridad chat ↔ web.** Todo lo nuevo de la migración 120 se cargaba por WhatsApp y no se veía en el dashboard: rinde esperado, avance en hectáreas, fechas de inicio/fin de cosecha, saldo por acopio y retiros. Ahora `GET /api/auth/harvest-summary` (misma ventana de campaña que el Resumen, scoping por `user_id` EN la query) los expone, y `HarvestCampaignsSummary` los muestra arriba de la tabla de camiones con los mismos filtros de campo/lote. El kg/ha usa la MISMA fórmula que el chat (sobre lo cosechado con avance parcial): dos pantallas no pueden dar dos rindes distintos del mismo lote.
- **Entrega duplicada.** Un POST idéntico llegó dos veces con 0,5 s de diferencia (proxy o fetch, no el runner) y la cosecha parcial se aplicó dos veces: el dedup por id no ve dos entregas con ids distintos. `withUserLock` acepta ahora un `dedupKey` (el TEXTO) y descarta el duplicado **solo mientras el original está en vuelo** — corriendo o encolado. Una respuesta repetida a un pending, que llega después, sigue pasando; los callbacks y audios no llevan clave. Los 3 controllers la pasan; el test-bot contesta `{deduped:true}` y WhatsApp un 200.
- **La confirmación ahora confirma.** Con "¿Confirmo gasto?" abierto, cualquier mensaje siguiente guardaba igual ("💡 Guardé el gasto antes de seguir"), así que la pregunta no confirmaba nada. Ahora una CONSULTA read-only se responde y la tarjeta reaparece ("⏳ Sigue pendiente", `renderPendingCard`), con el registro todavía sin guardar; lo mismo en el flow parado en el paso del lote. Un WRITE nuevo sigue guardando el anterior con aviso: perder el registro sería peor que guardarlo.
- **Consulta por acopio scopeada por contexto.** "cuánto entregué a Cargill" heredaba el lote de la conversación y respondía por un solo lote sin decirlo. Cuando la pregunta trae acopio/chofer/patente y el lote NO está en el texto (ni por nombre ni por pronombre), se descarta con log; nombrarlo explícitamente lo respeta.
- De paso: `llovieron` faltaba en `ACTION_VERB` (estaba `llovio`), así que una lluvia no contaba como pivot de un pending.
- Regresiones: `user-lock.dedup.test.ts` (5), `harvest-summary.routes.test.ts` (4, HTTP contra el backend real), `pipeline.integration.test.ts` § "confirmación que se respeta y consultas por acopio sin lote del contexto" (4). Suite: 2763 verdes.
