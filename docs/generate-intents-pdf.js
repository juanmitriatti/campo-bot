...
const newRules = [
  ...
  '  Cada load: driver_name, weight_kg, destination, destinatario, truck_plate, humidity_pct?, quality_metrics?.',
  '',
  '• UNIDADES (qq/tn/kg): qq=quintal=100kg, tn=tonelada=1000kg. "rindió 42 qq" → 4200 kg. "200 qq de soja" → 20000 kg.',
  '  El sistema convierte automáticamente vía normalizeToKg(). Mantener la unidad como la dijo el usuario; el código convierte cuando hace falta.',
  '',
  '• HUMEDAD por carga: capturar de "al 14%", "13.5 de humedad", "Britos 31.320 al 13.5%". Base AR: soja 13.5%, trigo 14%, maíz 14.5%.',
  '  Va en harvest_crop.loads[].humidity_pct (0-50%). Bot muestra "(14% hum)" + 💧 promedio + campaign_stats.yield.avgHumidity.',
  '',
  '• CALIDAD del grano: harvest_crop.loads[].quality_metrics (JSONB, crop-specific).',
  '  Soja: {oil_pct: 22.5}. Trigo: {protein_pct, gluten_pct, test_weight_kg_hl}. Girasol: {oil_pct: 44}.',
  '  CRÍTICO: pasarlas SOLO si el usuario las mencionó explícitamente — NO inventar valores.',
  '',
