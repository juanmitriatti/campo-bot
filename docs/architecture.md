...
- **`alert.service.js`** — Multi-channel alert delivery (Telegram-first, WhatsApp fallback). Dedup, retry with backoff, `alert_history` tracking.
- **`scheduler.js`** — node-cron: weekly summaries, daily weather alerts (HH:MM precision), low stock alerts (8AM), flow reminder tick (every minute). Argentina timezone.
...
### Agronomy (`agronomy/`)
- AgronomyHandler: activities, observations, weather, rainfall, agro reports (date range via desde/hasta), plot history queries (detail ≤15 rows, summary above), tacto/pregnancy checks, tacto summary, edit_last_activity, campaign lifecycle
...
PostgreSQL with migrations in `src/migrations/001-060_*.sql`. Timezone: `America/Argentina/Buenos_Aires` (migration 048). Migrations auto-run on app startup via `bootstrap()` → `runMigrations()`.
...
- `/api/auth/*` — Auth + dashboard (register, login, refresh, plans, observations, expenses, incomes, activities, stock, livestock — including PATCH edits, GET filters)
...
- Dashboard: Sidebar (desktop) + BottomNav (mobile), `view` state. Default: Overview (KPIs, quick actions, feed, alerts)
