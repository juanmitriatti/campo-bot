  ...
  return (
    <div className="p-4 md:p-6">
      <div className="flex items-baseline justify-between mb-4">
        <h2 className="text-lg font-semibold text-gray-800">Reportes agronómicos</h2>
        <span className="text-xs text-gray-400">PDFs generados desde el bot (se conservan 30 días)</span>
      </div>

      {loading && <div className="text-sm text-gray-500">Cargando reportes…</div>}
      ...
      {!loading && reports.length === 0 && !error && (
        <div className="text-sm text-gray-500 bg-gray-50 rounded-md p-6 text-center border border-dashed border-gray-300">
          Todavía no tenés reportes generados. Pedile al bot:
          <br />
          <span className="font-mono text-gray-700">"reporte agro del campo X"</span> o
            ...
            <tbody>
              {reports.map(r => (
                <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="py-2 pr-3 text-gray-800">{scopeLabel(r)}</td>
                  <td className="py-2 pr-3 text-gray-600">{periodLabel(r)}</td>
                  <td className="py-2 pr-3 text-gray-500 hidden md:table-cell">{formatDate(r.createdAt)}</td>
