...
### Escalera de escalamiento de pendings (Jul 2026)
Regla dura: **NUNCA repetir la misma pregunta más de 2 veces**. Nació del loop de prod "Me falta el lote. ¿Me lo decís?" × 3 (el usuario contestó "En otro lote" → se consumió como nombre → el handler re-seteó el pending CON el valor no resuelto en data → el cálculo de slots vacíos lo veía lleno → ni una respuesta válida podía llenarlo). Tres niveles, todos centrales (aplican a TODOS los pendings `missing[]` de todos los dominios — no hay nada por-handler):
1. **Razón + desenvenenado** (interceptor de no-progreso en `message-pipeline`, tras el re-ruteo): si el handler re-pide un slot que ESTE turno llenamos, se limpia el valor rechazado del pending re-seteado (fix del envenenamiento), se antepone la razón al re-ask ("🤔 No encontré el lote «5b». Si es un lote nuevo, decime *crear lote 5b*") y se loguea `[INTERCEPT] pending no-progress`. Además `buildAskPromptForMissing` reusa el askPrompt informativo del handler (con la lista de lotes) cuando el missing set no cambió, en vez de degradar a la plantilla genérica.
2. **Loop-breaker**: `PendingActivity.attempts` (+ `lastRejected`) cuenta re-asks sin progreso; se resetea con progreso parcial. Viaja por `applySideEffects` y el `PendingMirror` (sobrevive restarts).
3. **Rescate por el agente** (`escalatePendingToAgent`): con `attempts >= 2` el turno va al AGENTE con el pending completo — hint `RESCATE DE PENDING` (comando, datos, slot, valor rechazado, inventario real de lotes; pasa VERBATIM en agent.service, sin el truncado de 150 chars del pendingHint normal) + tag `[contexto: estaba completando ...]` inline en el texto (así el output-validator acepta los tokens que nosotros aportamos — mismo principio que el pronoun-expander). Regla de prompt "RESCATE DE PENDING" en agent-prompt-builder: no repetir la pregunta, ofrecer crear lo inexistente, o ejecutar la acción original si ya está completa. Regresiones en `pipeline.integration.test.ts` ("escalera de escalamiento").
