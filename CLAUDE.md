...
- **Caravana nombrada en una operación de GRUPO → la tool de grupo + `animal_ref`/`animal_refs`** (`record_livestock_death`, `remove_livestock`, `log_health_event`). El handler (`resolveNamedAnimals`) resuelve el animal (exacto, o **referencia corta por número si es única**: "la vaca 10" → 0000010), hereda lote y categoría del animal, y en la MISMA transacción descuenta el grupo y marca el animal (`setStatus`) o enlaza el evento a su ficha (`linkDomainEvent`, `animal_events.domain_event_id`). Una caravana inexistente es error visible, nunca fallback al grupo. Sanidad con caravanas que repite tipo+producto+ubicación del mismo día **se enlaza al evento existente** en vez de duplicarlo ("vacuné 3 con x29" → "la 1, 2 y 3 son las vacunadas"). Borrar el evento (`deleteDomainEvent` / `softDeleteDomainEvent`) borra los enlaces. QA prod 8 sep 2026: "se murió la vaca 10" descontó el grupo y el animal siguió "Activo" — no había forma de dar de baja un animal individual. Regresiones en `pipeline.integration.test.ts` § "caravanas nombradas".
- **Los slots de caravana (`newRfid`/`rfid`/`animalRef`) solo aceptan texto con forma de identificador** (`looksLikeAnimalId` en `animal-id.ts`, fuente única; guarda en el single-slot fallback Y en `identifyAnimal`). En prod "la 10 es macho, cambialo" entró como caravana nueva y reemplazó 0000010 por `LA10ESMACHOCAMBIALO`.
- **Corregir sexo/categoría/raza/nacimiento de un animal cargado = `update_animal`** (bot) / `PATCH /api/auth/animals/:id` (dashboard, botón "Corregir" del drawer), nunca `identify_animal` (solo caravana nueva). Sexo/raza/nacimiento son datos del animal (`AnimalService.updateAnimal`, evento `otro` from→to en la ficha). La **categoría mueve 1 cabeza entre grupos del mismo lote por el camino de grupo** (`transferAnimals` con `destCategory`, ajusta `count` y deja el movimiento) y después la ficha (`recategorize`: `group_id`, sexo implícito, evento `cambio_categoria` enlazado al movimiento). Invariante 16 intacto. Ojo: un comando nuevo son **4 registros**, no 3 — el cuarto es `commandToFeature` en `feature-gate.ts` (lo atrapa `animal-tools-registration.test.ts`).
- **`count` que en el texto aparece SOLO como superficie se dropea en el mapper** (`numberOnlyAppearsAsArea` en `lexicon.ts`, `COUNT_TOOLS` en `agent-response-mapper.ts`, log `AI_MAPPER DROP`): "En 5 hectáreas de ese lote tengo vacas" registró 5 vacas en prod. Sin count el handler pregunta "¿Cuántas cabezas?" con pending. "5 vacas en 5 ha" NO se toca.
- **`register_animal` sin ubicación hereda el lote del ÚNICO grupo vivo de la categoría** y se cuelga de él (`group_id` → `individualized_count`). Antes 10 altas quedaban "sin ubicación" al lado del grupo en La tapera. Con 2+ grupos no adivina.
...
- **Muletillas de nombre** ("agregar campo se llama X", "llamado X", "de nombre X") → `stripNameLeadIn()` en `lexicon.ts`, aplicado en el regex de `add_field`, el paso nombre del flow y el re-enunciado. En prod quedó un campo llamado «se llama el rehue».
- **QA E2E contra prod** (`src/testing/qa-prod-e2e.plan.json` + `qa-prod-e2e.ts --run`, cuenta admin de QA vacía, `.qa-env` gitignoreado): 53 mensajes de un productor mixto desde cero por el canal test-bot de prod. Correrlo después de un cambio grande de pipeline; el reporte queda en `qa-reports/`. Los hallazgos de la primera corrida (7 sep 2026) y sus regresiones viven en `pipeline.integration.test.ts` § "hallazgos del QA E2E de prod":
  - Las cargas **no achican** un rinde declarado: `updateYieldFromLoads` usa `GREATEST(yield_kg, Σ cargas)` (dos camiones parciales pisaban 384 tn con 58,5 tn).
  - `transfer_livestock` **hereda la raza** del grupo origen si el usuario no la nombró (creaba "Vaca" sin raza al lado de "Vaca Angus" y duplicaba grupos). El listado no muestra grupos en 0.
  - `READ_ONLY_TRIVIAL_COMMANDS` (intent-classifier): "mis recordatorios", "mis campos", "cómo vamos?"… se responden **en medio de un pending** (y el pending se re-pregunta) y **pasan el conversation lock** en vez de ir al agente. "cómo vamos?" pelado es `financial_report` por regex.
  - `impliesWholeGroup()` (lexicon): "vacuné **las** vacas del Sur" / "todos los terneros" = el grupo entero; sanidad y reproducción no preguntan "¿A cuántos?" (el evento se perdía en el pivot siguiente). Con número explícito no aplica.
  - Gasto de categoría de campo (arrendamiento, sueldos) con **varios campos** y ninguno nombrado → pending `missing:['field']` con botones `flow_field_*` (un tap sin flow activo que coincide con el slot del pending lo responde), nunca el picker de lotes de todos los campos.
  - Clima con homónimos (Junín BA / Mendoza): si UNA opción está en la provincia de un campo del usuario, se usa esa sin preguntar.
  - Validador: una **respuesta corta** (≤40 chars, ≤5 palabras) conserva el lote/campo que el usuario nombró en sus turnos recientes (`recentText`): "sembré en el Sur" → el agente preguntó el cultivo → "maíz" → `sow_crop(plot=Sur)` se dropeaba y la siembra caía en el lote del contexto. Un mensaje con acción propia no hereda. Segunda corrida del QA: 53/53.
  - **Segundo plan, mismo runner**: `qa-prod-ganaderia.plan.json` (62 mensajes, `--plan <archivo>`) recorre solo hacienda: grupos, feedlot con capacidad, capa individual con caravanas, lote de lectura pegado y reversión, con las existencias esperadas en cada `checks`. En `tap`/`buttons` un id terminado en `*` (`animal_batch_move_*`, `lv_loc_feedlot_*`) se resuelve por prefijo contra el último botón que mostró el bot — sin eso no se puede escribir ningún paso que dependa de un tap con token. El reporte sale como `qa-reports/<plan>-<timestamp>.md`. Primera corrida (9 sep 2026): 42/60; hallazgos reales y regresiones en `pipeline.integration.test.ts` § "hallazgos del QA E2E de ganadería":
    - **Cantidad + animal pivotea un pending de UBICACIÓN aunque sea corto** (`looksLikeLivestockCount` en `conversation-guards.ts`, usado por el escape estricto de message-pipeline salvo en slots financieros): con "¿en qué lote?" abierto, "en 5 hectáreas del Sur tengo vaquillonas" se consumió como lote «5» y las 20 vaquillonas se perdieron. Además `extractPlotToken` ya no toma "en N" como lote si N va seguido de una unidad (ha/kg/mm/días…).
    - **El agente NUNCA reescribe una caravana**: el validador (sin flag, regla dura) exige que `animal_ref`/`animal_refs` estén respaldadas por el texto expandido (forma normalizada ≥8 chars contenida, o valor sin ceros igual a un token numérico). Sin respaldo y con un único número en el texto, lo reemplaza por ese número (`AI_VALIDATOR OVERRIDE`); si no, lo dropea. "dónde está la 12?" llegó como `0000000013` porque el modelo leyó el reemplazo en el historial.
    - **Caravana retirada se explica** (`findRetiredIdentification`): `query_animal` resuelve exacto → ref corta → retirada, y en ese caso antepone "🔁 La caravana X ya no está vigente: fue reemplazada por *Y* el DD/MM" a la ficha actual. Sigue sin resolver en el lookup vigente (una baja por esa caravana es error visible).
    - **`set_livestock_price` no confía en el sentido del agente**: "los toros salieron 2 palos c/u" llegó como `kind=income`; si con ese tipo no hay movimiento sin precio, usa el que SÍ existe y el tipo del movimiento decide gasto/ingreso (log `[INTERCEPT] set_livestock_price kind override`).
    - "acordame/recordame" son verbos de acción (`ACTION_VERB`): un recordatorio nuevo con el "¿a qué hora?" del anterior abierto ya no se traga como la hora.
    - **"corral 2" como respuesta a "¿a qué lote o corral?" es un corral** (`LivestockService.resolveLocation` mira el prefijo `corral` en `plotName`): el destino de un pending llega por el slot genérico de lote y se buscaba un lote llamado «corral 2». Segunda corrida: 58/62, todo lo demás eran supuestos del plan.
    - **Pesaje con artículo definido = grupo entero** (`impliesWholeGroupIgnoringWeight` en lexicon: ignora "N kg"/"promedio N" antes de decidir; cualquier otro número sigue siendo cantidad explícita). "pesé los terneros, promedio 160 kg" pasó dos corridas porque el agente adivinó `animals_weighed` y falló la tercera cuando lo omitió: el artículo decide, no el agente.
    - **"cuándo se vacunó?" / "dónde está la 13?" son consultas aunque haya un slot numérico abierto** (`QUERY_INTENT`: `cuando se \w+`, `cuando (fue|vacun…)`, `donde (esta|estan)`; un "cuando" pelado NO, porque "cuando sea" contesta "¿a qué hora?"). Con "¿A cuántos animales?" abierto se consumía como la cantidad y el bot solo re-preguntaba. Tercera corrida: 60/62; cuarta (96dc01b): **62/62**.
- **Validador + audio**: un campo/lote REAL del usuario cuenta como mencionado si una palabra del texto *suena* igual (`soundsLikeToken` en `entity-matcher.ts`: h muda, b/v, s/z/c, ll/y, g/j, qu/k, 1 edición, tokens ≥4 letras). Whisper transcribió "El Rehue" como "El Regué" y el validador vetaba la resolución correcta del agente. Loguea `AI_VALIDATOR KEEP (fonético)`. No reemplaza a `STT_DOMAIN_CORRECTIONS` (que solo toca palabras sin sentido en español).
...
- **Audio**: `DEFAULT_WHISPER_PROMPT` (vocabulario agro AR en contexto de frase) + capa determinística `STT_DOMAIN_CORRECTIONS` en `text-normalizer.js` — SOLO palabras sin sentido en español (cero falsos positivos; jamás palabras reales). Manglings nuevos van AHÍ. La mayoría de los "wrong tool" del path de audio son mala transcripción, no handlers. **El prompt de Whisper NO lleva cantidades ni precios**: Whisper lo trata como transcripción previa y con un audio corto lo copia — en prod (8 sep 2026) "compré yerba" salió "Compré 10 novillas por 500" (calcado del ejemplo del prompt) y se registró un grupo de 10 vaquillonas + $5.000.000 inventados. Regresión en `src/services/audio/__tests__/whisper-prompt.test.ts`.
...
- **QA E2E de siembra/cosecha contra prod** (`qa-prod-siembra-cosecha.plan.json`, 45 mensajes; informe `qa-reports/qa-siembra-cosecha-informe.md`, 9 sep 2026, 39/45 + 6 bugs reales; migración 121). Reglas que salieron de ahí — regresiones en `pipeline.integration.test.ts` § "hallazgos del QA E2E de siembra/cosecha" y `overview.integration.test.ts`:
  - **Cosecha parcial: rate × ha COSECHADAS, y se acumula.** "cosechamos 40 ha, rindió 42 qq/ha" = 168 tn (no × 100 sembradas = 420); el día siguiente con otras ha SUMA (`addPlotCropYield`), no pisa. Sin hectáreas ("rindió 42 qq/ha", "sacamos 130 tn") sigue siendo el total de la campaña.
  - **`src/utils/harvest-campaign-kg.ts` es la fuente ÚNICA de kilos cosechados para el dashboard** (Resumen `plots[].harvestKg`, `harvestsMonthly`, `yieldByCrop`): una fila por CAMPAÑA = GREATEST(rinde declarado, Σ cargas netas, Σ quantity de eventos). Sumar por evento doblaba el rinde de una cosecha de dos días. kg/ha por cultivo = Σ kg / Σ ha, nunca promedio de ratios.
  - **`src/utils/mass-units.ts` es la fuente ÚNICA de kg/tn/qq** (`normalizeToKg` del mapper se re-exporta desde ahí). `addGrainStock` convierte a la unidad del ítem existente en vez de fallar ("cargar 130 tn" sobre soja en kg).
  - **Venta de grano con comprador: el lote es OPCIONAL y nunca abre `income_flow`.** Se deduce de la única campaña reciente del cultivo (`findPlotsWithCrop`); con 2+ queda a nivel campo con aviso. El flow "¿En qué lote?" se tragaba "retiré 5 tn…" como lote y perdía el comprador. `buyer`/`price_status` viajan por el partial, el flow y `commitFinancialFlowFieldLevel`.
  - **Venta "a fijar" = ingreso completo con monto 0** (`price_status:'a_fijar'`, CHECK relajado en migración 121), nunca un `income_partial` que pide "¿Cuánto fue?".
  - **`add_stock` lleva `plot`**: el insumo entra al depósito del campo, el gasto vinculado queda en el lote que nombró el usuario.
  - **Una sola botonera por respuesta**: el costo de cosechar se ofrece DESPUÉS del tap de stock (`harvest-cost-offer.ts`, compartido por el handler y el pipeline). El flete usa las tn netas en camión (sin el silo propio), no el rinde declarado. `campaign_stats` divide por ha cosechadas cuando hay avance parcial (misma fórmula que la confirmación). Cerrar con avance parcial pide confirmación.
  - `getCampaignTotals` (ranking) acota la plata a `end_date`, no a `harvested_at`: el costo de cosecha con fecha de fin quedaba afuera y el ranking decía "margen $0". Las campañas omitidas se nombran.
  - Fechas DATE de Postgres se formatean SIEMPRE con `formatDateAR` (llegan como medianoche UTC; `new Date(d)` en zona AR corre un día). `getDailyRainfallTotal` acumula por `rainfall_date`, y la alerta dice la fecha. El reset del test-bot borra `task_reminders`/`suggestion_events`/`form_sessions`.
  - Corridas tras el deploy `f54e024` (9 sep 2026): 42/45 → 42/46 → **45/46**; todos los fallos restantes eran regex del plan (la frase de confirmación es aleatoria: «Anotado» / «registrado» / «guardado»; `Monto:` no es pedir monto).
  - **Dedup por CONTENIDO en vuelo** (`withUserLock(key, fn, {dedupKey, onDuplicate})`): el mismo TEXTO mientras el original está corriendo o encolado se descarta con log `[INTERCEPT] dedup`. En la corrida 5 un POST idéntico llegó dos veces con 0,5 s (proxy/fetch, no el runner) y la cosecha parcial se aplicó dos veces — el dedup por id no ve dos entregas con ids distintos. **Solo en vuelo**: una respuesta repetida a un pending, que llega después, sigue pasando; callbacks y audios no llevan clave. Los 3 controllers la pasan.
  - **La tarjeta "¿Confirmo gasto/ingreso?" se respeta**: una CONSULTA read-only en el medio se responde y la tarjeta reaparece (`renderPendingCard`, "⏳ Sigue pendiente") con el registro SIN guardar; ídem el flow parado en el paso del lote. Un WRITE nuevo sigue guardando el anterior con aviso (perderlo es peor). Antes cualquier mensaje lo guardaba y la pregunta no confirmaba nada.
  - **`query_harvest_loads` con acopio/chofer/patente descarta el lote del CONTEXTO**: "cuánto entregué a Cargill" es por acopio, no por lote; si el lote no está en el texto (nombre o pronombre) se dropea con log. Nombrarlo explícitamente lo respeta.
  - **Paridad chat ↔ dashboard**: `GET /api/auth/harvest-summary` (misma ventana de campaña que el Resumen, scoping por `user_id` EN la query) expone rinde esperado, avance en ha, fechas de inicio/fin, saldo por acopio y retiros; lo rinde `HarvestCampaignsSummary` arriba de la tabla de camiones. **El kg/ha usa la MISMA fórmula que el chat** (sobre lo cosechado con avance parcial): dos pantallas no pueden dar dos rindes del mismo lote. Regresiones en `harvest-summary.routes.test.ts` y `user-lock.dedup.test.ts`.
...
- **Per-user serialization** (`user-lock.ts`): mensajes del mismo usuario en orden, usuarios distintos en paralelo. In-process (single-replica). Además **dedup por contenido en vuelo**: `withUserLock(key, fn, {dedupKey: <texto>, onDuplicate})` descarta el mismo texto mientras el original corre o está encolado (dos entregas HTTP con ids distintos, que el dedup por id no ve). Solo en vuelo — una respuesta repetida a un pending sigue pasando.
...
- **Login / verificación de email / reset de contraseña (revisado desde cero, Sep 2026 — migración 119)**:
  - **`email-normalizer.ts` es la fuente ÚNICA de normalización de emails de cuenta** (trim + minúsculas) y `AuthRepository.findByEmail` busca con `LOWER(email)`. Antes registro (verbatim), login (exacto) y forgot (lowercase) usaban tres criterios: un usuario registrado como "Juan@Gmail.com" no podía recuperar la contraseña nunca (200 mudo sin email). La migración normaliza las cuentas viejas salvo colisión.
  - **`one-time-token.ts` es la fuente ÚNICA de tokens de reset/verificación**: sha256 con lookup exacto (antes bcrypt, que obligaba a traer los 50 tokens pendientes de TODO el sistema y compararlos uno a uno: un link válido pero viejo caía fuera de la ventana y tardaba segundos). Los bcrypt emitidos antes del deploy siguen valiendo hasta vencer (fallback `$2%`). Excluye usuarios borrados. `confirmVerificationToken` es idempotente (`alreadyVerified`): el mismo link tocado dos veces ya no dice "inválido".
  - **Cuentas `suspended`/`disabled` (admin) no entran ni refrescan** (403 con mensaje; se chequea DESPUÉS de validar la contraseña para no servir de oráculo) y no reciben link de reset. `getUserById`/`findByEmail` excluyen `deleted_at`.
  - **`auth-rate-limit.ts`**: login cuenta fallos por email (10/15 min → 429 con `Retry-After`; un reset exitoso libera el bloqueo), forgot 3 por email/15 min, reset/verify por IP. Por email y no por IP porque detrás de Railway sin `trust proxy` la IP es la misma para todos. Cada 429 loguea `[AUTH RATE]`.
  - **Frontend `api/client.ts`**: refresh single-flight (N requests con el access vencido disparaban N refresh con el MISMO token: el primero rotaba y los demás cerraban la sesión) y los endpoints públicos (`PUBLIC_AUTH_ENDPOINTS`) nunca disparan refresh ni redirigen (una contraseña equivocada con un token viejo mostraba "Sesión expirada").
  - Tokens de sesión y de email se purgan en el cleanup diario (`authTokensCleanupTick`); antes ninguna tabla tenía purga. Regresiones: `auth-lifecycle.integration.test.ts` (ciclo completo con mailer mockeado), `auth-rate-limit.test.ts`, `auth.service.test.ts`.
...
- **Formularios estructurados (Ago 2026, migración 105; 6 formularios desde Sep 2026)**: siembra, cosecha, gasto, ingreso, labor y alta de hacienda se pueden cargar por form de pantalla única (Telegram Mini App; WhatsApp Flows). `FormDefinition` en `src/forms/form-definitions.ts` es la fuente ÚNICA (render React + validación server + Flow JSON) y es un **registro**: `action`, `label`, `settingKey`, `fields`, `crossCheck`; el comando lo arma `src/forms/form-commands.ts`. **Formulario nuevo = entrada en `FORM_DEFINITIONS` + builder + handler que emite `offerForm` + setting + fila del picker**; `form-registry.test.ts` falla si falta alguno. El submit fuerza `confirm_before_save:false` (el form ya es la confirmación). Opciones dinámicas (lotes/cultivos) via `src/forms/form-options.ts` (`computeFormOptions`) — misma fuente para el form web y para hornear el `flow_action_payload.data` del Flow. Sesiones token-based en `form_sessions` (30 min, un uso). El botón se ofrece vía sideEffect `offerForm` (suprimido en bulkMode — invariante 7) y comando trivial `formulario`. El submit entra por `routeCommand` con `withUserLock` — sin IA. **WhatsApp Flows: código completo** (`appendFormOffer` emite `BotResponseItem` tipo `flow` + `sendFlow` en `whatsapp.js`; endpointless, sin RSA; respuesta por `nfm_reply` → `submitForm(..., {flowResponse:true})` que re-arma los 5 slots de cargas con `unflattenFlowPayload`), **gateado por settings `WHATSAPP_FLOW_ID_SOW`/`_HARVEST` (grupo bot)** — sin flow_id publicado en Meta sigue dark. Los Flows se crean/validan/publican con `npx tsx src/scripts/publish-whatsapp-flows.ts` (nunca pegar JSON a mano; un Flow publicado es inmutable → `--recreate`). El `complete` del Footer DEBE llevar `${form.x}` por cada componente (con `{}` el nfm_reply vuelve vacío) y el DatePicker habla `YYYY-MM-DD`, no epoch. Todo path loguea `[FORM]`. Ver docs/features/forms.md + docs/operations.md § "WhatsApp — checklist de activación".
...
- `src/services/overview.service.ts` → `GET /api/auth/overview?field_id=&season=` — payload completo del Resumen (resultado ARS **y** USD en paralelo, ranking de categorías, **qué vendiste** por producto con tn y $/tn, **margen por cultivo**, lluvia por mes de la campaña **y de la anterior**, lotes con gasto/ingreso/**$/ha/rinde**, presupuestos del mes, pendientes, hacienda por categoría, feed, `counts` para los badges del nav). El viejo `/dashboard` ("este mes vs el anterior") y `/analytics` **se borraron** (Sep 2026): no les quedaba ningún consumidor.
  ...
  - **El picker de campañas lista solo campañas que pueden tener datos**: `campaignsSince(earliestDataDate(userId), {pinned: season})` en `/overview` — de la actual hacia atrás hasta la del registro más viejo (gasto/ingreso/evento/lluvia, sin borrados), tope 10, y la seleccionada en la URL siempre queda. Sin datos → solo la actual, y el `CampaignPicker` muestra texto en vez de un select de una opción. Antes `recentCampaigns(4)` ofrecía 22/23, 23/24, 24/25 vacías a un usuario nuevo (feedback prod, sep 2026).
