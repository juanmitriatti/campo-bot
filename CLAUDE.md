...
- **QA E2E de siembra/cosecha contra prod** (`qa-prod-siembra-cosecha.plan.json`, 45 mensajes; informe `qa-reports/qa-siembra-cosecha-informe.md`, 9 sep 2026, 39/45 + 6 bugs reales; migración 121). Reglas que salieron de ahí — regresiones en `pipeline.integration.test.ts` § "hallazgos del QA E2E de siembra/cosecha" y `overview.integration.test.ts`:
  ...
  - Corridas tras el deploy `f54e024` (9 sep 2026): 42/45 → 42/46 → **45/46**; todos los fallos restantes eran regex del plan (la frase de confirmación es aleatoria: «Anotado» / «registrado» / «guardado»; `Monto:` no es pedir monto). **Hallazgo abierto**: en la corrida 5 un mismo POST al test-bot llegó DOS veces con 0,5 s de diferencia (proxy o fetch, no el runner) y la cosecha parcial se aplicó dos veces (avance 80/100). El dedup por id no cubre dos entregas con ids distintos de un TEXTO; un dedup por contenido choca con las respuestas repetidas legítimas de los pendings, así que no se agregó. Si aparece en WhatsApp real, resolverlo con ventana corta + "hay un write idéntico en vuelo para este usuario".
