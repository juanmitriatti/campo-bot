...
- **QA E2E contra prod** (`src/testing/qa-prod-e2e.plan.json` + `qa-prod-e2e.ts --run`, cuenta admin de QA vacía, `.qa-env` gitignoreado): 53 mensajes de un productor mixto desde cero por el canal test-bot de prod. Correrlo después de un cambio grande de pipeline; el reporte queda en `qa-reports/`. Los hallazgos de la primera corrida (7 sep 2026) y sus regresiones viven en `pipeline.integration.test.ts` § "hallazgos del QA E2E de prod":
  ...
  - **Segundo plan, mismo runner**: `qa-prod-ganaderia.plan.json` (62 mensajes, `--plan <archivo>`) recorre solo hacienda: grupos, feedlot con capacidad, capa individual con caravanas, lote de lectura pegado y reversión, con las existencias esperadas en cada `checks`. En `tap`/`buttons` un id terminado en `*` (`animal_batch_move_*`, `lv_loc_feedlot_*`) se resuelve por prefijo contra el último botón que mostró el bot — sin eso no se puede escribir ningún paso que dependa de un tap con token. El reporte sale como `qa-reports/<plan>-<timestamp>.md`. Primera corrida (9 sep 2026): 42/60; hallazgos reales y regresiones en `pipeline.integration.test.ts` § "hallazgos del QA E2E de ganadería":
    ...
    - **"cuándo se vacunó?" / "dónde está la 13?" son consultas aunque haya un slot numérico abierto** (`QUERY_INTENT`: `cuando se \w+`, `cuando (fue|vacun…)`, `donde (esta|estan)`; un "cuando" pelado NO, porque "cuando sea" contesta "¿a qué hora?"). Con "¿A cuántos animales?" abierto se consumía como la cantidad y el bot solo re-preguntaba. Tercera corrida: 60/62.
