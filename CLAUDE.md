...
- **`tool-definitions.ts`** — 36 Anthropic tool definitions grouped by domain: Financial (2), Activities (6), Observations (2), Reports (5), Field/Plot Mgmt (5), Sharing (4), Stock (8), Documents (3), System (1). Each tool has typed `input_schema` with enum validation for categories. All registration tools include an optional `event_date` param (YYYY-MM-DD) for user-mentioned dates.
