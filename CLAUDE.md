...
- **`tool-definitions.ts`** — 65 Anthropic tool definitions grouped by domain: Financial (2), Activities (11 — includes `active_crop`, `close_campaign`, `campaign_stats`), Observations (2), Reports (5), Field/Plot Mgmt (11), Sharing (4), Stock (8), Documents (3), Livestock (8), Feedlot/Corral (7), System (1), plus 3 additional tools. Each tool has typed `input_schema` with enum validation for categories. All registration tools include an optional `event_date` param (YYYY-MM-DD) for user-mentioned dates. Query/report tool descriptions include expected output data (hectares, weight, grade, etc.) and example trigger phrases for better tool selection.
...
- **`agronomy/`** — AgronomyHandler (activities, observations, weather, rainfall, agro reports, plot history queries (detail ≤15 rows, summary above), tacto/pregnancy checks, tacto summary queries, edit_last_activity, campaign lifecycle); `normalizeActivityFilter()` maps AI filter strings to DB event_type values. `generate_agro_report` supports date range (`desde`/`hasta`) and defaults to current week; shows all observations and activities (no caps). **Tacto**: `log_tacto` registers pregnancy checks with `pregnant_count`/`open_count`/`uncertain_count` on `domain_events`; auto-computes total from parts and open from total-pregnant-uncertain; shows pregnancy rate %. **Tacto summary**: `tacto_summary` queries aggregated tacto data with optional field/plot/date filters; shows global summary with per-lote breakdown (pregnancy rates) or single-plot detail. DB query `getTactoSummary()` in `expenses.js`, typed wrapper in `agronomy.repository.ts`. **Edit activity**: `edit_last_activity` finds user's last matching activity (optional eventType/crop filter) and updates its plot, crop, or date. **Campaign lifecycle**: `harvest_crop` sets `harvested_at` on `plot_crops` (campaign stays open for post-harvest activities); `close_campaign` sets `end_date` to permanently close; `campaign_stats` aggregates activities/expenses/incomes/yield/profitability via `CampaignStatsService`. Three states: ACTIVE (`end_date IS NULL AND harvested_at IS NULL`), HARVESTED (`end_date IS NULL AND harvested_at IS NOT NULL`), CLOSED (`end_date IS NOT NULL`). `CropService.startCrop()` auto-closes harvested campaigns when re-sowing; accepts optional `sowedHectares` param for partial-plot sowing. **`active_crop`** AI tool: with plot → single plot's crop; without plot → lists ALL active crops across user's fields (filtered by crop name if provided, e.g. "soja?" → all plots with soja). Shows per-plot detail by default when ≤15 results; regex fallback for explicit detail requests on larger lists. `getAllActiveCrops()` query in `expenses.js` with `field_members` access control. **Sowed hectares**: `sow_crop` tool accepts optional `hectares` param; stored in `plot_crops.sowed_hectares`. `active_crop` shows "Sembradas: X ha (de Y ha totales)" when partial; totals use `COALESCE(sowed_hectares, area_hectares)`
...
- **`stock/`** — Full inventory management: StockRepository (atomic movements with row locking), StockService (auto-resolve warehouse/field, fuzzy product search, grain stock), StockHandler (9 chat commands; `check_stock` shows expanded detail per product when ≤15 items, compact one-liner above), StockPurchaseService (expense→stock entry suggestion + grain entry), StockDeductionService (activity→stock deduction), StockAlertService (low stock alerts daily at 8AM). Feature-gated to `pro_plus`/`enterprise` plans.
...
- **`livestock/`** — Full cattle/hacienda management: LivestockRepository (atomic movements via `applySingleMovement` + `applyTransferMovement`, both using `FOR UPDATE` row locks; transfers lock both groups in consistent UUID order to prevent deadlocks), LivestockService (category normalization with plural/accent aliases, location resolution via `resolveLocation()` supporting both plots and corrals — never auto-creates, find-or-create groups, auto-classifies same-location+different-category as `recategorizacion`), LivestockHandler (8 chat commands). Event-sourced model: `livestock_groups` (state projection, `plot_id` OR `corral_id` with CHECK constraint) + `livestock_movements` (immutable audit log with 7 types: entrada/salida/transferencia/muerte/nacimiento/recategorizacion/ajuste). DB-level `CHECK` constraint on `livestock_movements` enforces valid endpoint configurations per movement type. Feature-gated to `pro_plus`/`enterprise` plans. 19 service tests.
- **`feedlot/`** — Feedlot (intensive cattle management) with corrals: FeedlotRepository (CRUD for feedlots + corrals, access via `field_members`), FeedlotService (corral resolution across user's feedlots, auto-resolve when user has 1 feedlot), FeedlotHandler (7 commands: create_feedlot, list_feedlots, delete_feedlot, create_corral, list_corrals, delete_corral, rename_corral). Max 1 feedlot per campo (UNIQUE field_id). Corrals unique by name within feedlot. Cross-type transfers (lote↔corral) supported. Feature-gated to `livestock` (same as livestock). Migration 055.
...
PostgreSQL with migrations in `src/migrations/001-056_*.sql`. Database timezone set to `America/Argentina/Buenos_Aires` (migration 048). Schema initialized by `init.sql` (mounted in Docker). **Migrations run automatically on app startup** via `src/scripts/run-migrations.ts` (called from `bootstrap()` in `src/app.ts` before `app.listen()`). The runner tracks applied migrations in a `schema_migrations` table, applies pending files in alphabetical order (each in its own transaction), and aborts startup on failure. All migrations are idempotent (`IF NOT EXISTS` / conditional `DO` blocks). Set `RUN_MIGRATIONS_ON_START=false` to disable (e.g. for unit tests). The runner can also be invoked standalone via `npx tsx src/scripts/run-migrations.ts`. Key tables: `users` (includes `telegram_id`, `province`, `last_name` columns), `fields` (includes `province`, `lat`, `lng`, `polygon`, `location_method`), `plots`, `expenses` (includes `expense_date`, `edited_by`, `expense_type`, `product`, `quantity`, `unit`), `incomes` (includes `income_date`, `edited_by`), `budgets`, `rainfall`, `domain_events` (activities, includes `event_date`, `edited_by`, `stock_deduction_status`, `pregnant_count`, `open_count`, `uncertain_count`), `agro_observations` (includes `observation_date`), `warehouses` (per-field storage), `stock_items` (inventory with `current_quantity`, `min_stock`, `grade`, `humidity_pct`), `stock_movements` (entrada/salida/ajuste with links to `expense_id`/`domain_event_id`), `user_settings`, `global_settings`, `ai_usage`, `conversation_logs` (includes `tool_calls` JSONB, `agent_mode`, and `channel` columns), `conversation_events`, `conversation_state`, `unparsed_messages`, `refresh_tokens`, `observation_history`, `field_members` (sharing), `field_invites` (invite codes), `alert_history` (alert delivery tracking), `activity_dictionary` (admin-editable activity synonyms for AI agent prompt), `map_tokens` (single-use tokens for map polygon drawing), `livestock_groups` (UUID pk, per-location state projection: user_id/field_id/plot_id OR corral_id/category/breed/count, `chk_location_exclusive` CHECK ensures exactly one of plot_id/corral_id, partial unique indexes on (plot_id,category,breed) and (corral_id,category,breed)), `livestock_movements` (UUID pk, immutable audit: movement_type enum/source_group_id/dest_group_id/count with DB-level `chk_movement_endpoints` CHECK constraint), `feedlots` (max 1 per campo via UNIQUE field_id, soft delete), `corrals` (multiple per feedlot, UNIQUE feedlot_id+name, soft delete), `plot_crops` (campaign tracking: crop, season_year, season_type, start_date, end_date, `harvested_at`, `yield_kg`, `yield_notes`, `sowed_hectares`; unique active per plot via partial index).
